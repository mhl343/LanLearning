{
 "cells": [
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "Now let's write a for loop with conditional statements! Create an array of integers ranging from 1-5 (inclusive). If the number is odd, add ```1``` to the number. If the number is even, add ```2``` to the number. "
   ]
  },
  {
   "cell_type": "code",
